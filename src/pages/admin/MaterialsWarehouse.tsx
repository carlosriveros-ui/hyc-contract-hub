...
import { materials, movements, getMaterial, getUser } from "@/data/mock";
  ...
  return (
    <AppShell title="Materiales & Bodega" subtitle="Inventario y movimientos">
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        ...
        <KpiCard label="Movimientos hoy" value={movements.length} icon={ArrowDown} accent="success" />
        ...
        <TabsContent value="movimientos" className="mt-4">
          <div className="bg-surface rounded-lg border border-border shadow-card p-5">
            ...
            <ol className="space-y-3">
              {movements.map((mv) => {
                const cfg = movementIcons[mv.type];
                const Icon = cfg.icon;
                  ...
                  <li key={mv.id} className="flex gap-3">
                    ...
                    <div className="flex-1 min-w-0 pb-3 border-b border-border">
                      <div className="flex justify-between gap-2 flex-wrap">
                        <p className="text-sm font-semibold">{cfg.label} · {mat?.name}</p>
                        <span className="text-xs text-muted-foreground">{new Date(mv.date).toLocaleTimeString("es-CO", { hour: "2-digit", minute: "2-digit" })}</span>
                      </div>
                      <p className="text-xs text-muted-foreground mt-0.5">
                        {mv.qty} {mat?.unit} · {mv.origin} → {mv.destination}
                      </p>
                      <p className="text-xs text-muted-foreground">por {getUser(mv.userId)?.name}</p>
                    </div>
                  </li>
                );
              })}
